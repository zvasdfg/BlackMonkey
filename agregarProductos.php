...
<!--                    <h1>Modern Business - A Bootstrap 3 Template</h1>-->
    ...
    include('conexion.php');
							 ...
							 $status = "";
							  ...
							  $tamano = $_FILES["imagenP"]['size'];
							  $tipo = $_FILES["imagenP"]['type'];
							  $archivo = $_FILES["imagenP"]['name'];
							 
							  if ($archivo != "") {
								  // guardamos el archivo a la carpeta files, si no existe la tienes q crear antes de subir los archivos.
								   $destino =  "files/".$archivo;
									  if (copy($_FILES['imagenP']['tmp_name'],$destino)) {
										  $status = "Archivo subido con exito: <b>".$archivo."</b>";
										  //aqui va el insert para guardar en la tabla los campos
			$descripcion=$_POST['txtDescripcion'];	
			$cantidad=$_POST['txtCantidad'];
			$precio=$_POST['txtPrecio'];
			$oper_insert="insert into productos (descripcion, cantidad, precio, imagen) values ('$descripcion', '$cantidad', '$precio','$archivo')";
							  ...
							  echo "<input type='image' name='imageField' src='files/$archivo'/>";
							  		
						echo $_POST['txtDescripcion'],"   ",$archivo;
